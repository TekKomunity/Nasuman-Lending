...
public function loanPlansMob(Request $request)
    {
        $pageTitle      = 'Loan Plans';
        $emptyMessage   = 'No Plan Yet';
        $plans          = LoanPlan::active()->get();
                $loans           = UserLoan::where('user_id', $request->user_id)->with('plan')->orderBy('id', 'DESC')->get();

        return  response()->json(["data"=>$plans,"loans"=>$loans]);
    }
    
    
    public function notificationsMob(Request $request)
    {

                $loans           = EmailLog::where('email_to', $request->email_id)->orderBy('id', 'DESC')->get();

        return  response()->json(["data"=>$loans]);
    }
    
    public function check_kyc_Mob(Request $request)
    {
        $pageTitle      = 'Loan Plans';
        $emptyMessage   = 'No Plan Yet';
        $plans          = User::where("id",$request->user_id)->get();
    ...
    public function loanFormSubmitMob(Request $request)
    {
     //   $loan   = session()->get('loan');
                ...
                if($item['type'] == 'file'){
                    try {
                        ...
                        $notify[] = ['error', 'Could not upload your ' . $key];
                               ...
                               return response()->json(["data"=>"error","message"=>"Files are Missing","status"=>"400"]);
...
//        session()->forget('loan');
        $notify[]=['success',' successfully'];
        return response()->json(["data"=>$shortCodes,"message"=>"Loan Applied Successfully","status"=>"200"]);
