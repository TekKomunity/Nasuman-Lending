...
public function register_mob(Request $request)
    {
        
       // return response()->json($request->all());
      //  $this->validator($request->all())->validate();
        $exist = User::where('mobile',"91".$request->mobile)->first();
        if ($exist) {
            $notify[] = ['error', 'The mobile number already exists'];
            return  response()->json(["data"=>"","status"=>"402","message"=>"The mobile number already exists"]);
        
            
        }

        event(new Registered($user = $this->create_mobile($request->all())));
//return response()->json($request->all());
        $this->guard()->login($user);

        return  response()->json(["data"=>$user,"status"=>"200","message"=>"Account Created Successfully"]);
    //    return $this->registered($request, $user)
      //      ?: redirect($this->redirectPath());
    }
    ...
    protected function create_mobile(array $data)
    {
        ...
        $user->username         = trim($data['mobile']);
        $user->ref_by           = $referUser ? $referUser->id : 0;
        $user->country_code     = "91";
        $user->mobile           = "91".$data['mobile'];
        $user->address = [
